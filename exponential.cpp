...
int exponential(std::vector<int> array, int target) {
    ...
    while(step < array.size()){
        ...
        if (target < array[step]){
            return binary(array, target, step/2, step-1);
        }
    ...
    return binary(array, target, step/2, array.size() - 1);
...
int main() {
    ...
    int target = 5;
    int index = exponential(array, target);
